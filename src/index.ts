...
import { startDiscovery, testDiscovery, startSignalDiscovery } from "./discovery/index";
...
let signalDiscoveryRunning = false;
let shuttingDown = false;

let sendCron: ScheduledTask | null = null;
let enrichmentCron: ScheduledTask | null = null;
let discoveryCron: ScheduledTask | null = null;
let signalDiscoveryCron: ScheduledTask | null = null;
...
/* -----------------------------
      Discovery (runs immediately + cron every 6 hours)
   ------------------------------ */

  await safeRun(
    () => discoveryRunning,
    (v) => (discoveryRunning = v),
    async () => {
  ...
  discoveryCron = cron.schedule("0 */6 * * *", async () => {
  ...
  /* -----------------------------
      Signal-Driven Discovery (runs in background, then every 6 hours offset)
   ------------------------------ */

  safeRun(
    () => signalDiscoveryRunning,
    (v) => (signalDiscoveryRunning = v),
    async () => {
      await startSignalDiscovery();
    },
    "SignalDiscovery",
  ).catch((err) => logger.error({ err }, "SignalDiscovery background task failed"));

  signalDiscoveryCron = cron.schedule("30 */6 * * *", async () => {
    if (shuttingDown) return;
...
async function shutdown(signal: string) {
  ...
  try {
    ...
    if (discoveryCron) discoveryCron.stop();
    if (signalDiscoveryCron) signalDiscoveryCron.stop();
