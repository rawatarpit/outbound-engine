...
export interface BrandIntent {
  ...
  // New fields for enhanced intent management
  search_queries?: string[]; // JSONB array of search strings per intent
  exclusion_patterns?: string[]; // JSONB array of patterns to reject
  conversion_rate?: number; // float, auto-computed (0-1)
  last_refined_at?: string; // timestamp
}
...
export async function getClientMembers(
  ...
  if (error) {
    logger.warn({ error: error.message }, "Failed to get client members");
    return [];
  }
