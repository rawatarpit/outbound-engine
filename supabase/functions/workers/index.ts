  ...
  try {
    ...
    if (path === "/metrics" || path === "/metrics/") {
      if (req.method === "GET") {
        ...
        let query = supabase.from("brand_profiles").select("id, brand_name, product, discovery_count_today, sent_count").eq("client_id", clientId);
        if (brandId) {
    ...
    if (triggerMatch && req.method === "POST") {
      ...
      if (workerName === "discovery") {
        const { data: brands } = await supabase.from("brand_profiles").select("id").eq("client_id", clientId).limit(1);
