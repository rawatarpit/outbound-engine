...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { supabase, user, clientId } = auth;
    ...
    if (!brandId) {
      if (path === "/overview" || path === "/overview/") {
        ...
        const { data: companies } = await supabase.from("companies").select("status").in("brand_id", brandIds);
        const stages = {
      ...
      if (path === "/companies" || path === "/companies/") {
        if (req.method === "GET") {
          ...
          let query = supabase.from("companies").select("id, brand_id, name, domain, website, status, industry, estimated_value, deal_value, source, created_at, updated_at, owner_name, owner_title, owner_linkedin", {
            count: "exact"
          }).in("brand_id", brandIds).range(offset, offset + limit - 1).order("created_at", {
            ascending: false
          });
    ...
    if (companyIdMatch) {
      ...
      if (companyPath === "" || companyPath === "/") {
        ...
        if (req.method === "PATCH" || req.method === "PUT") {
          ...
          if (body.status && allowedStatuses.includes(body.status)) {
            const { data: member } = await supabase.from("client_members").select("role").eq("user_id", user.id).maybeSingle();
            if (!member || ![
              "owner",
              "admin"
